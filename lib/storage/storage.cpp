...
// ── Extract the UID field from a CSV line ─────────────────────────────────────
...
bool storage_init() {
    ...
    const char* needed[] = {
        ...
        FILE_NET_MODE,  FILE_SYSCFG   // <-- ADDED FILE_SYSCFG
    };
    for (auto p : needed) {
...
int storage_read_fare() {
    if (!_lock()) return DEFAULT_FARE_AMOUNT;
    File f = LittleFS.open(FILE_SYSCFG, "r");
    if (!f) { _unlock(); return DEFAULT_FARE_AMOUNT; }
...
StorageResult storage_write_fare(int fare_amount) {
    ...
    File f = LittleFS.open(FILE_SYSCFG, "w");
    ...
    LOG_INFO("STORAGE", "Dynamic Fare updated to: %d", fare_amount);
    return STORAGE_OK;
}
...
StorageResult storage_remove_uid(const char* path, const char* uid) {
    ...
    LittleFS.rename(FILE_TEMP, path);
    _unlock();
...
StorageResult storage_atomic_delete_sent(size_t bytes_sent) {
    ...
    LittleFS.rename(FILE_TEMP, FILE_TX_LOG);
