...
StaffAuthResult auth_check_staff_uid(const char* uid){
    ...
    // PHASE 8 SECURITY NOTE: 'uid' will be hashed before checking staff files.
...
StaffAuthResult auth_verify_pin(const char* file, const char* uid, const char* pin){
    ...
    // PHASE 8 SECURITY NOTE: 'uid' will be hashed before looking up the PIN.
...
StudentValidResult auth_validate_tap(const char* uid, unsigned long now, int* out_cnt){
    ...
    // PHASE 8 SECURITY NOTE: 'uid' will be hashed before hitting the whitelist.
    ...
    // PHASE 8 SECURITY NOTE: 'uid' will be hashed before hitting the blacklist.
    ...
    // PHASE 8 SECURITY NOTE: 'uid' will be hashed before searching tx.log.
