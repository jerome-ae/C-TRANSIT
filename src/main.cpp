...
static void handle_ready_keypad(char key);      // NEW — keypad in STATE_READY
...
void setup() {
    ...
    xTaskCreatePinnedToCore(rfid_ui_task, "rfid_ui",
                            STACK_SIZE_APP, nullptr, PRIORITY_RFID,
                            nullptr, CORE_APP);
...
static bool boot_hardware() {
    ...
    display_show_2line("  C-TRANSIT   ", "  Booting...  ");
    ...
    // ── REMOVED: hardcoded driver/admin seed ─────────────────────────────────
    // Drivers and admins are provisioned ONLY via backend downlink:
    //   ADD:DR,<UID>,<PIN>  or  ADD:AD,<UID>,<PIN>
    // or via Registration Mode (Phase 6).
    // ─────────────────────────────────────────────────────────────────────────
...
// =============================================================================
//  handle_ready_keypad — NEW
//  Handles keypad input while terminal is in STATE_READY (active shift).
//
//  '#' — driver logout: flush pending taps, clear session, lock terminal
//  'A' — cycle network mode: AUTO → WIFI ONLY → GSM ONLY → AUTO
// =============================================================================
static void handle_ready_keypad(char key) {
    switch (key) {
        ...
        default:
            // All other keys ignored in READY state
            break;
    }
}

// =============================================================================
//  handle_offline_locked_tap  (unchanged)
// =============================================================================
static void handle_offline_locked_tap(const char* uid) {
